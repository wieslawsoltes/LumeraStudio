...
| Persistence | D1 projects/members/invites/versions; local SQLite runtime; named checkpoint restore; scene import/export; IndexedDB in Pages mode | Large asset storage, R2 uploads, project sizes beyond 1.5 MB, native Katana/USD/Alembic exchange |
| Collaboration | Server mode: 2.5-second polling, presence, field patches, revision compare-and-swap, independent-field merge, same-field conflict copy flow, roles and expiring invites, local recovery draft | Pages is browser-local only. WebSocket transport, CRDT/OT, live cursors, enterprise SSO/SCIM, audit retention, disaster recovery, clustering or production security certification remain unimplemented |
...
The original hosted site starts owner-private. Project invites grant application membership only; visitors also need permission to enter that hosted site. The bundled local server intentionally binds to loopback and represents one local user. It is not an Internet authentication server. For multi-user self-hosting, mount `server/api.js` behind a trusted identity provider and use the SQL adapter contract; do not expose the local identity adapter publicly.

## Validation status

The 37 Node regression tests pass locally and in GitHub Actions. They cover scene validation, graph cycle checks and evaluation, undo/redo, keyframe serialization/interpolation, transformed geometry, CPU shading changes, SQL persistence, authorization and invitation expiry, stale revisions, checkpoints, pending-edit compaction, synchronization races, ZIP records, Pages paths, local transport and recovery isolation.

Chromium 153.0.8010.12 passed the real Pages smoke test in GitHub Actions on September 20, 2026. Checks cover subpath assets, save/reload, rename persistence, second-tab loading, stale-revision conflicts, explicit local sharing, no server API calls and no page errors. The screenshot was visually inspected. The browser test forces CPU fallback; it is not a WebGPU test. See the `lumera-validation` CI artifact for its screenshot and JSON report.

The static build and all four package tarballs pass. The original Sites Worker build was validated during the original implementation, not rerun by the dependency-free Pages workflow. Local Chromium navigation was blocked by administrator URL policy, so browser execution was performed in GitHub Actions.

WebGPU pipeline execution/readback, physical-GPU/driver tests, production asset-scale benchmarks, comprehensive interaction/accessibility coverage and enterprise security qualification remain unperformed. Static WGSL review is not equivalent to runtime GPU qualification. Optional WebMCP registration is feature-detected and is not qualified here.

## GitHub Pages storage

The static edition stores projects and checkpoints in IndexedDB, with namespaced recovery and transactional revision checks. It does not provide multi-user invitations, authentication or server database hosting. Different devices and browser profiles do not share projects. Export Lumera JSON files before clearing browser data. The server adapters and SQL migrations remain in the repository for server-backed deployment.
